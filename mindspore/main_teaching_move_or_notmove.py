...
def create_custom_dataset(data_path, batch_size=16, repeat_size=1):
    ...
    """
    # Veri setini oluştur
    dataset = ds.ImageFolderDataset(data_path, class_indexing={"hareketli": 0, "hareketsiz": 1})

    # Görüntü işlemleri
    def decode_image(data):
        """
        Görüntüleri çözümlemek için gerekirse numpy array formatına dönüştürme.
        """
        # Eğer veri zaten numpy array ise, doğrudan kullan
        if isinstance(data, np.ndarray):
            return vision.Decode()(data)
        # Eğer veri Tensor ise, önce numpy array'e dönüştür
        return vision.Decode()(data.asnumpy())

    trans = [
        decode_image,  # Görüntüleri çözümle
...
class AdvancedClassificationNet(nn.Cell):
    def __init__(self):
        ...
        self.fc = nn.SequentialCell([
            ...
            nn.Dense(4096, 3)  # hata var 2 ye çekmmem lazımdı 
        ])
...
data_path = "/home/eren/İndirilenler/Eren_dataset_iki"  # Kendi veri seti yolunuzu buraya koyun
train_dataset = create_custom_dataset(data_path)

print("Model eğitimi başlıyor...")
model.train(epoch=40, train_dataset=train_dataset, callbacks=[LossMonitor()], dataset_sink_mode=True)  # Daha fazla epoch
print("Eğitim tamamlandı!")

# Modeli kaydet
save_checkpoint(network, "move_or_not_move_detection.ckpt")
