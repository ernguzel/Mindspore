...
def create_custom_dataset(data_path, batch_size=16, repeat_size=1):
    """
    İnsan ve sandalye veri setini yükler ve MindSpore formatına dönüştürür.
    ...
    """
    # Veri setini oluştur
    dataset = ds.ImageFolderDataset(data_path, class_indexing={"person": 0, "chair": 1})
...
class AdvancedClassificationNet(nn.Cell):
    def __init__(self):
        ...
        self.fc = nn.SequentialCell([
            ...
            nn.Dense(4096, 2)  # Sınıflar (İnsan ve Sandalye)
        ])
...
model.train(epoch=50, train_dataset=train_dataset, callbacks=[LossMonitor()], dataset_sink_mode=True)  # Daha fazla epoch
